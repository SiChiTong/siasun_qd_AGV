...
void paramter_define()
{
    ...
    wheelParamter.line_slowest_time = 0.1; //0.1
    wheelParamter.motor_max_rotationl_velocity_soft = 2000;
    wheelParamter.motor_min_rotationl_velocity_soft = 150;
    wheelParamter.WHEEL_DIAMETER = 150.0;
    wheelParamter.REDUCATION_RATIO = 15.0;
    wheelParamter.wheel_acceleration_time = 0.3636;//加减速2m，慢速0.15m
...
void Location_AGV()
{
	...
	{
		//AGV_Current_Coor_InWorld = PGV150_coor;   //更新全局坐标
		AGV_Current_Coor_InWorld = Odom_Calib(AGV_Target_Velocity_InAGV.velocity_x, AGV_Current_Velocity_InAGV.angular_velocity_angle);    //无二维码部分进行里程推算
...
//if( (abs(angle_error) < 0.25) && (abs(Error_Coor_InAGV.x_coor) < 2 ||abs(Error_Coor_InAGV.y_coor) < 2))
	//PID_error = 0;
...
//}
 	 if (AGV_Target_Velocity_InAGV.velocity_x > 1100)
        PID_result = HI * (PID_error - error_next) + HP * PID_error + HD * (PID_error - 2 * error_next + error_last);
 	 else if (AGV_Target_Velocity_InAGV.velocity_x <= 1100)
