...
extern float input_distance_abs;
...
/*
	 if(abs(AGV_Current_Coor_InWorld.y_coor-900)<7 && abs(AGV_Current_Coor_InWorld.x_coor-700)<7 && stop_flage>8000){
	   x = 700;//700/0
	   y = 2000;//3300/500
	   stop_flage=0;
	}

	else if(abs(AGV_Current_Coor_InWorld.y_coor-2000)<7 && abs(AGV_Current_Coor_InWorld.x_coor-700)<7 &&  stop_flage>8000 ){
		x = 0;
		y = 2000;
		stop_flage=0;
	}

	else if(abs(AGV_Current_Coor_InWorld.x_coor) < 7 && abs(AGV_Current_Coor_InWorld.y_coor - 2000) < 7&&  stop_flage>8000 ){
		x = 0;
		y = 900;
		stop_flage=0;
	}
	else if(abs(AGV_Current_Coor_InWorld.x_coor ) < 7 && abs(AGV_Current_Coor_InWorld.y_coor - 900) <7 &&  stop_flage>8000 ){
		x = 700;
		y = 900;
		stop_flage=0;
	}
*/
		 if(abs(AGV_Current_Coor_InWorld.x_coor) < 7 && abs(AGV_Current_Coor_InWorld.y_coor - 3300) < 7&&  stop_flage>8000 ){
		x = 0;
		y = 700;
		stop_flage=0;
	}
	else if(abs(AGV_Current_Coor_InWorld.x_coor ) < 7 && abs(AGV_Current_Coor_InWorld.y_coor - 700) <7 &&  stop_flage>8000 ){
		x = 0;
		y = 3300;
		stop_flage=0;
	}
   ...
   // Destination_Coor.angle_coor = Destination_Coor_InWorld.angle_coor; //屏幕获取目标角度
	...
	if(x_coor_last != Destination_Coor_InWorld.x_coor ||y_coor_last != Destination_Coor_InWorld.y_coor)   //判定目标点是否发生变化
	{
		current_distance = 0.0;
		acc_distance = const_distance = dec_distance = slowly_distance = 0.0;
		virtual_agv_coor_init_flag = 0;
		State = No_Interpolation;           //重新计算插补
	}

	x_coor_last =Destination_Coor_InWorld.x_coor;
	y_coor_last = Destination_Coor_InWorld.y_coor;
